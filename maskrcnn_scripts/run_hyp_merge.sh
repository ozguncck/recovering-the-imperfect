...
if [ "x${snap}" != "x" ]; then
    python3 ${train_file} train --dataset=${dataset_dir} --subset train_final --logs=${log_dir} --weights=${log_dir}/mask_rcnn_diventura_${snap}.h5 --loss=aleatoric

else
    python3 ${train_file} train --dataset=${dataset_dir} --subset train_final --logs=${log_dir} --loss=aleatoric
fi
